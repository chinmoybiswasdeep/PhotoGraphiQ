...
Fock adapter supports correlated resources, conditional photon counting and
homodyne, Kerr and cubic gates, ideal ladder operations, and finite cat/cubic
resource injection with nonlinear feed-forward and cutoff diagnostics. We derive the conventions, measurement updates, teleportation
...
This article describes the implemented version 0.2.0 repository. Its contributions
...
examples and data. The non-Gaussian extension adds finite-resource injection,
conditional wavefunction updates, physical heralding, occupation-aligned state
metrics and independent Fock-space validation.
...
uses a global total-photon cutoff and supports ideal adaptive homodyne
conditioning through an explicit wavefunction adapter. CV-flow is checked for a supplied total order; the software does
...
LaTeX table and figures. Running
\texttt{python experiments/non\_gaussian.py} regenerates the non-Gaussian
convergence CSV, figures and local environment record. The manuscript uses the official Quantum document class,
...
Noisy Fock homodyne, mixed-state input resources, GKP error correction,
...
\bibitem{marek2011}
P. Marek, R. Filip, and A. Furusawa,
``Deterministic implementation of weak quantum cubic nonlinearity,''
\href{https://doi.org/10.1103/PhysRevA.84.053802}{Phys. Rev. A \textbf{84}, 053802 (2011)}.
...
\href{https://doi.org/10.1103/PhysRevA.93.022301}{Phys. Rev. A \textbf{93}, 022301 (2016)}.
\end{thebibliography}
...
CI configuration covers Python 3.11--3.14 and includes pytest, Ruff, mypy and
wheel/source-distribution builds on every version. Static checking targets Python 3.12 because the
