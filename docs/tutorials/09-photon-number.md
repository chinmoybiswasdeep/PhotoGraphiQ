...
Select one branch of a correlated two-mode Fock resource. Install the [required extras](../getting-started/installation.md) first.

## Theory

Destructive PNR projects one mode onto a number state. The branch probability follows Born’s rule and the remaining mode is normalized conditionally. All examples use [hbar=2 conventions](../getting-started/conventions.md).
...
Run `python examples/tutorials/09-photon-number.py` from the repository root. The script
uses a fixed seed or deterministic postselection where measurement is involved.

```python
--8<-- "examples/tutorials/09-photon-number.py"
```
...
```text
--8<-- "docs/tutorials/outputs/09-photon-number.txt"
```

The probability is one half; the remaining state has one photon given the selected count. Postselection does not make the branch occur on every experimental shot.
...
Remove postselection, repeat many shots, and compare empirical count frequencies with one half.
