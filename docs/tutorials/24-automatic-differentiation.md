...
Compare a JAX derivative with an analytical rotation derivative. Install the [required extras](../getting-started/installation.md) first.

## Theory

A fixed-topology numerical program can propagate parameter derivatives. Measurement normalization and branch likelihood require distinct treatment; discrete sampling needs an estimator. All examples use [hbar=2 conventions](../getting-started/conventions.md).
...
Run `python examples/tutorials/24-automatic-differentiation.py` from the repository root. The script
uses a fixed seed or deterministic postselection where measurement is involved.

```python
--8<-- "examples/tutorials/24-automatic-differentiation.py"
```
...
```text
--8<-- "docs/tutorials/outputs/24-automatic-differentiation.txt"
```

The derivative is of the finite-Fock expectation with respect to angle. Here it agrees with -sin(theta), independently of a stochastic estimator.
...
Add vacuum loss with a trainable transmissivity. Compare selected-branch gradients with gradients of its probability.
