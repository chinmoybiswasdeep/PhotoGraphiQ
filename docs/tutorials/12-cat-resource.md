...
Prepare an even cat and inspect parity. Install the [required extras](../getting-started/installation.md) first.

## Theory

An even cat combines coherent amplitudes +alpha and -alpha. Only even photon occupations remain in the ideal normalized superposition. All examples use [hbar=2 conventions](../getting-started/conventions.md).
...
Run `python examples/tutorials/12-cat-resource.py` from the repository root. The script
uses a fixed seed or deterministic postselection where measurement is involved.

```python
--8<-- "examples/tutorials/12-cat-resource.py"
```
...
```text
--8<-- "docs/tutorials/outputs/12-cat-resource.txt"
```

Parity +1 reflects support on even occupations. Wigner interference depends on alpha and phase-space resolution.
...
Switch to odd parity, then use a complex alpha. Explain the rotated interference pattern.
