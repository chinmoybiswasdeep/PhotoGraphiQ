# Compiling a nonlinear resource injection

## Goal

Compile a circuit-level cubic gate into an executable resource pattern. Install the [required extras](../getting-started/installation.md) first.

## Theory

Gaussian operations plus cubic phase form a universal CV gate set. A finite-energy implementation retains resource filtering; the polynomial front end currently accepts quadrature degrees up to four. All examples use [hbar=2 conventions](../getting-started/conventions.md).
...
Run `python examples/tutorials/22-universal-compilation.py` from the repository root. The script
uses a fixed seed or deterministic postselection where measurement is involved.

```python
--8<-- "examples/tutorials/22-universal-compilation.py"
```
...
```text
--8<-- "docs/tutorials/outputs/22-universal-compilation.txt"
```

The cubic gate is supplied by a prepared nonlinear resource and Gaussian corrections. Its finite envelope remains part of the simulated result.
...
Inspect a Kerr synthesis report at steps 1 and 4 before compiling it. Estimate resource cost and separate synthesis from injection errors.
