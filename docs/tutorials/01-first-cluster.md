...
Build a three-mode resource and inspect its nullifier noise. Install the [required extras](../getting-started/installation.md) first.

## Theory

For a graph adjacency A, ideal nullifiers are p-Aq. Finite momentum squeezing gives residual variance exp(-2r). A resource without measurements has no classical outcomes. All examples use [hbar=2 conventions](../getting-started/conventions.md).
...
Run `python examples/tutorials/01-first-cluster.py` from the repository root. The script
uses a fixed seed or deterministic postselection where measurement is involved.

```python
--8<-- "examples/tutorials/01-first-cluster.py"
```
...
```text
--8<-- "docs/tutorials/outputs/01-first-cluster.txt"
```
